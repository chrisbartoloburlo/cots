...
import examples.openverse.model.AuthorizeApiOauth2Request
import examples.openverse.model.OAuth2AuthenticationSuccessful
import examples.openverse.model.OAuth2RegistrationSuccessful
import examples.openverse.model.RegisterApiOauth2Request
...
def apply(baseUrl: String = "http://localhost:50280/v1") = new AuthTokensApi(baseUrl)
}

class AuthTokensApi(baseUrl: String) {

  /**
   * Expected answers:
   *   code 201 : OAuth2AuthenticationSuccessful (OK)
   * 
   * @param authorizeApiOauth2Request 
   */
  def authorizeApiOauth2(authorizeApiOauth2Request: AuthorizeApiOauth2Request
): Request[Either[ResponseException[String, Exception], OAuth2AuthenticationSuccessful], Any] =
    basicRequest
      .method(Method.POST, uri"$baseUrl/auth_tokens/token/")
      .contentType("application/json")
      .body(authorizeApiOauth2Request)
      .response(asJson[OAuth2AuthenticationSuccessful])

  /**
   *  register_api_oauth2 is an API endpoint to register access to the API via OAuth2.  Upon registering, you will receive a `client_id` and `client_secret`, which you can then use to authenticate using the standard OAuth2 Client Credentials flow. See the Register and Authenticate section for instructions on registering access to the API via OAuth2.  > **WARNING:** You must keep `client_secret` confidential, as anybody with your > `client_secret` can impersonate your application.  Authenticated users have higher rate limits than anonymous users. Additionally, by identifying yourself, you can request Openverse to adjust your personal rate limit depending on your organization's needs.   You can refer to the cURL request samples for examples on how to consume this endpoint. 
   * 
   * Expected answers:
   *   code 201 : OAuth2RegistrationSuccessful (OK)
   *   code 400 : String (Bad Request)
   * 
   * @param format 
   * @param data 
   */
  def registerApiOauth2(format: String, data: RegisterApiOauth2Request
